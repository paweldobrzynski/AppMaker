...
**ACCEPTED** — promoted from WU-003 to `decisions/` on 2026-05-09. The original draft remains immutable at `.appmaker/work-units/wu-003/runs/2026-05-09T14-27-51Z/output.md` for audit.

Lifecycle: `DRAFT` (in run dir, immutable) → **`ACCEPTED`** (current — this file at `decisions/ADR-002-interview-phase.md`) → `AMENDED` (if modified by future amendment work_unit, per Amendment Process in constitution).
...
| 2026-05-09 | WU-003 (Codex consistency review) | DRAFT | Three consistency fixes pre-promote: (1) D1 `--skip` output now satisfies D3's structural requirements (non-empty `unresolved_ambiguities[]` + populated `override`); (2) Matt Pocock pinned commit hash recorded explicitly: `b843cb5ea74b1fe5e58a0fc23cddef9e66076fb8`; (3) brownfield `glossary.md` location specified as `.appmaker/glossary.md` (kernel-managed per ADR-001 §D3). 351 → 375 lines. |
| 2026-05-09 | WU-003 (promote) | ACCEPTED | Promoted from `.appmaker/work-units/wu-003/runs/2026-05-09T14-27-51Z/output.md` to `decisions/` after Codex REVIEW PASS. Status flip DRAFT → ACCEPTED in promoted copy; original draft immutable in run dir. Mapping doc concurrently promoted to `docs/reference/matt-pocock-pattern-mapping.md`. |

---

**End of ADR-002 (ACCEPTED — WU-003 promoted 2026-05-09).**
