...
**ACCEPTED** — promoted from WU-007 to `decisions/` on 2026-05-10. The original draft remains immutable at `.appmaker/work-units/wu-007/runs/2026-05-10T08-07-32Z/output.md` for audit. One mapping entry concurrently appended to `docs/reference/matt-pocock-pattern-mapping.md` (Entry 4; existing 3 entries unchanged).

Lifecycle: `DRAFT` (in run dir, immutable) → **`ACCEPTED`** (current — this file at `decisions/ADR-005-work-unit-decomposition.md`) → `AMENDED` (if modified by future amendment work_unit, per Amendment Process in constitution).
...
| 2026-05-10 | WU-007 (Codex pre-promote review × 2) | DRAFT | Round 1 (3 blocking + 1 non-blocking) — AC #17 split (existing fields strict-diff vs new decomposition fields conceptual); D2 extended with HITL/AFK slice typing decision; pairs_to_diff 5→7 (added D2↔D5 + D3↔D5); §10 L10 wording. Round 2 (2 blocking semantic) — D4(b) `work_unit_subtype: decomposition` target correction (decomposition WU itself, NOT every materialized slice; clarified across lines 156, 173, 319); D4 enum scoped down from `null|decomposition|review|retro|amendment|interview|prd` to `null|decomposition|future_extension` with candidate names moved to OQ-9 (avoiding silent acceptance of future taxonomy without dedicated ADR). 392 → 393 lines. |
| 2026-05-10 | WU-007 (promote) | ACCEPTED | Promoted from `.appmaker/work-units/wu-007/runs/2026-05-10T08-07-32Z/output.md` to `decisions/` after Codex REVIEW PASS. Status flip DRAFT → ACCEPTED in promoted copy; original draft immutable in run dir. Mapping entry concurrently appended as Entry 4 to `docs/reference/matt-pocock-pattern-mapping.md` (existing 3 entries — grill-me, grill-with-docs, to-prd — diff-verified unchanged). |

---

**End of ADR-005 (ACCEPTED — WU-007 promoted 2026-05-10).**
