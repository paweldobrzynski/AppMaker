...
**DRAFT** — produced by WU-006 at `.appmaker/work-units/wu-006/runs/2026-05-09T23-44-24Z/output.md`, awaiting promote.

Lifecycle: `DRAFT` (in run dir, immutable) → **`ACCEPTED`** (after human + Codex review; promotion step copies this file to `decisions/ADR-004-prd-synthesis.md` and appends one entry to `docs/reference/matt-pocock-pattern-mapping.md`) → `AMENDED` (if modified by future amendment work_unit, per Amendment Process in constitution).
