...
**DRAFT** — produced by WU-005 at `.appmaker/work-units/wu-005/runs/2026-05-09T21-52-08Z/output.md`, awaiting promote.

Lifecycle: `DRAFT` (in run dir, immutable) → **`ACCEPTED`** (after human + Codex review; promotion step copies this file to `decisions/ADR-003-schema-format-and-artifact-schemas.md` and the three secondary schema files to `.appmaker/schemas/`) → `AMENDED` (if modified by future amendment work_unit, per Amendment Process in constitution).
