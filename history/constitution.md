...
**ACCEPTED — initial v1 promoted from WU-002 on 2026-05-09; AMENDED on 2026-05-09 by WU-004 adding R18 (Interview Phase as required first lifecycle stage, per ADR-002).**

The original v1 draft remains at `.appmaker/work-units/wu-002/runs/2026-05-09T11-18-09Z/output.md`. The amendment draft (this file's source) remains at `.appmaker/work-units/wu-004/runs/2026-05-09T16-24-46Z/output.md`. Both immutable for audit.

Lifecycle: `DRAFT` v1 (WU-002 run dir) → `ACCEPTED` v1 (project root, WU-002 promote) → **`AMENDED`** (current — this file at project root, WU-004 amendment promote) → further `AMENDED` if modified by future amendment work_unit, per Amendment Process.
...
**Why:** Without a structured uncertainty-reduction stage at project start, every downstream work_unit inherits ambiguous requirements and either spirals into rework (a silent R13 violation) or pretends ready when it is not (a silent R12 violation). Interview Phase is the project-level mechanism that operationalizes R13 (uncertainty reduction or verified change) and protects R12 (no silent fallbacks) at the lifecycle entry point. Constitution must not allow this gate to be optional. Full design rationale, alternatives, and propagation semantics are in ADR-002.

**How to apply:** AppMaker kernel refuses to create any work_unit (other than the dedicated interview work_unit itself) until `.appmaker/interview-result.yaml` exists with `readiness.status` in `{ready, ready_with_override}`. The four-state readiness enum (`ready`, `needs_more_input`, `reject`, `ready_with_override`) and the `ready_with_override` propagation rules are defined in ADR-002 §§D3, D6. Skipping requires explicit human break-glass `appmaker interview --skip --reason="<skip rationale>"`, which produces a structured `interview-result.yaml` whose `readiness.status` is forced to `ready_with_override` with a project-wide ambiguity entry; the skip is human-only, never silent, and aligned with R6's break-glass discipline.
...
| R18 | auto-check + human-review | Kernel checks `.appmaker/interview-result.yaml` exists with `readiness.status` in `{ready, ready_with_override}`; if status is `ready_with_override`, kernel also enforces unresolved-ambiguity propagation per ADR-002 §D6; `--skip` is human-only break-glass aligned with R6's discipline |
...
| 2026-05-09 | WU-002 (promote) | ACCEPTED | Promoted from `.appmaker/work-units/wu-002/runs/2026-05-09T11-18-09Z/output.md` to project root after Codex REVIEW PASS (status flip, R7 wording fix re-validated, length 382 lines with 18-line buffer). No content changes between draft and promoted; only Status section and this row added. |
| 2026-05-09 | WU-004 (amendment) | AMENDED → ACCEPTED | Added rule R18 enforcing ADR-002 §D1: Interview Phase is required first lifecycle stage; `.appmaker/interview-result.yaml` with `readiness.status` in `{ready, ready_with_override}` is gate for any other work_unit creation. R1–R17 unchanged in title, body, Why, and How to apply (textual diff verified). All six Founding Principles (P1–P6) still encoded. Verification Hooks table extended with R18 row (auto-check + human-review). Strict reading of Amendment Process applied — R18 semantically touches R5 (fail-closed gates) and R13 (uncertainty reduction); dual human review with documented `solo_execution_exception` per WU-004 work-unit.yaml. WU-002 (constitution v1) and WU-003 (ADR-002 promote) remain valid; AppMaker meta-project grandfathered (no retroactive invalidation). Pre-amendment: 383 lines; post-amendment within ≤ 400 hard cap. |

---

**End of Constitution (AMENDED — WU-004 promoted 2026-05-09; current version includes WU-002 v1 plus WU-004 R18 amendment).**
