...
Status: 25 skills (19 core + afk + status + token-audit + next + phase + debt). v0.2.30 — ponytail adoption (legibility, not new epistemics): deliberate shortcuts leave an `appmaker:debt <ceiling> -> upgrade: <path>` marker harvested by `/appmaker:debt` (Tier-1 grep via `debt-json.sh`) into a ledger; review/checklist warn on bare markers and gain a YAGNI/over-engineering lens (`skills/yagni-ladder.md` + `build_intensity` dial). Multi-host portability + always-on mode deliberately rejected (clash with the Claude-Code-layer, opt-in positioning). v0.2.29 — visual layer (legibility, not new epistemics): markdown-native wireframe-first step in `prd` (mermaid + ASCII, view-of-PRD never source of intent) catches intent drift before AC; `review` emits a non-gating visual recap aggregating existing evidence; Studio gains a Wireframes panel over a new `wireframes-json.sh`. v0.2.28 — scan-first browser E2E + anti-placebo test-validity gate (Checkly/Playwright-AI webinar distillation): TDD grounds UI E2E locators in a live-DOM snapshot instead of hallucinating them, and review/checklist gate against placebo tests (skipped/tautology/commented/no-assert) that AI writes to please a "make it pass" prompt. v0.2.27 — AppMaker Studio MVP: local Node server + static cockpit over `status-json.sh` and `phase-plan.sh --json`, showing project state, evidence, and phase waves without adding a separate source of truth. Phase execute remains the wave-by-wave subagent runtime contract. optional gstack browser runtime still serves QA/design evidence. 34 smoke suites. Test harness covers hooks + glossary-extract + version SoT (with release-target), fresh init materialization, skill body diet, architecture research, brownfield impact audit, GSD/gstack-inspired planning gates, engine JSON API, Studio UI, phase dry-run/execute/runtime, gstack browser adapter, browser-e2e grounding, test-validity gate, rigor config, checklist Execution Record gates, and side-effect invocation boundaries.
...
33. **Plugin hardening through real-world use (caseman BPS session, 2026-05-11).** Sequential patches from production session findings:
    - **v0.2.28:** Scan-first browser E2E + anti-placebo test-validity gate. Distilled from the Checkly "modern E2E with Playwright and AI" webinar (2026-06-14). Two webinar findings AppMaker did not yet cover: (1) blind E2E generation hallucinates locators because the LLM never saw the live DOM — you must scan the running app (Playwright/gstack MCP page snapshot → accessibility tree into context) BEFORE generating; (2) when told "make it pass", AI weakens/skips/comments asserts → fake-green placebo tests. Patch — all in existing skills + supporting refs, NO new top-level skills:
      1. **`resources/appmaker/skills/tdd/browser-e2e.md`** (new supporting ref): scan-first contract for UI/browser ACs. Build `appmaker/features/<NNN>/app-map.md` (pages + API routes + flows — the webinar's `setup.md`/`testplan.md` pattern), scan flow via `$B goto`/`$B snapshot -i` (or Playwright MCP `browser_snapshot`), generate E2E with role/label locators traced to snapshot lines (never invented), bounded run→fail→fix repair loop (max 3, never weaken asserts). `/appmaker:tdd` planning + verification reference it.
      2. **`resources/appmaker/skills/tdd/test-validity.md`** (new supporting ref): anti-placebo gate. Tier 1 deterministic grep over changed test files (`.skip`/`.only`/`xit`/`fit`/tautology `expect(true)`/`assert(true)`/commented `// expect`/no-assert bodies), Tier 2 judgment (would the test go red on regression?). Wired into `/appmaker:tdd` verification step 7, `review-contract.md` (items 23–24, FAIL on placebo guarding an AC + FAIL on blind E2E), and `/appmaker:checklist` (new Test validity + Browser E2E grounding rows with inline rg detection).
      3. **Reuses existing gstack flags** (`gstack_enabled`, `gstack_browse_bin`, `gstack_required_for_ui_qa`) — no new config surface. No browser available → E2E AC is `human-review:`/blocked, never fabricated. Smoke: `test-browser-e2e-grounding.sh` + `test-test-validity-gate.sh` (28 → 30 suites). Deliberately NOT done: bundling Playwright/Checkly itself or a synthetic-monitoring/post-deploy lifecycle (out of governance-layer scope; webinar's Checkly-cloud handoff stays a user choice).
    ...
    - **v0.2.13:** `/appmaker:next` lifecycle dispatcher skill. Resolves real-session friction: side-effect skills (`prd`, `decompose`, `tdd`, `archive`, etc.) have `disable-model-invocation: true` (v0.2.9 audit fix preventing silent writes), so agent cannot chain phases automatically — user has to type each slash command manually. `/appmaker:next` (itself `disable-model-invocation: true`, single user entry) detects current state via filesystem, picks next phase deterministically (10-row state machine), confirms via AskUserQuestion, then emits the exact slash-command handoff. v0.2.21 caseman evidence showed the earlier "invoke target via Skill tool" assumption fails with `Skill appmaker:tdd cannot be used with Skill tool due to disable-model-invocation`; current contract forbids that path.
