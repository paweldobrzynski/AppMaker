...
README_22=$(grep -cE '← 22 dirs \(17 core \+ afk \+ status \+ token-audit \+ next \+ phase\)' "$README" || true)
[ "$README_22" -ge 1 ] && README_22_OK="yes" || README_22_OK="no"
assert_eq "README layout shows ← 22 dirs (17 core + afk + status + token-audit + next + phase)" "yes" "$README_22_OK"
...
README_TOTAL=$(grep -cE '^22 written:' "$README" || true)
[ "$README_TOTAL" -ge 1 ] && README_TOTAL_OK="yes" || README_TOTAL_OK="no"
assert_eq "README narrative explicit '22 written:' total" "yes" "$README_TOTAL_OK"

# --- DESIGN layout (pcrit-004) ---

DESIGN_22=$(grep -cE '← 22 dirs \(17 core \+ afk \+ status \+ token-audit \+ next \+ phase\)' "$DESIGN" || true)
[ "$DESIGN_22" -ge 1 ] && DESIGN_22_OK="yes" || DESIGN_22_OK="no"
assert_eq "DESIGN layout shows ← 22 dirs (17 core + afk + status + token-audit + next + phase)" "yes" "$DESIGN_22_OK"
